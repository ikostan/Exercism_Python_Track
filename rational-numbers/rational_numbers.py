...
class Rational:
    ...
    def __pow__(self, power):
        ...
        # real (floating-point) number x is the quotient (a^x)/(b^x), which is a real number.
        elif power >= 0 and type(power) == float:
            return Rational((self.numer ** power), (self.denom ** power))
        # Exponentiation of a rational number r = a/b to a
        # negative real (floating-point) number x is the quotient (a^x)/(b^x), which is a real number.
        elif power < 0 and type(power) == float:
            return Rational((self.denom ** abs(power)), (self.numer ** abs(power)))
